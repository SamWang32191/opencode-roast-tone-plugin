...
- `roast`: 目前的基準款，尖銳、節奏快、帶明顯 punchline。文案在本次改動中保持與現行版本一致，不順手重寫。
- `dry`: 冷面、克制、少表演，吐槽像順手補刀。
- `deadpan`: 更平、更硬，笑點靠語氣反差，不靠情緒起伏。
- `mentor`: 嚴格但建設性，保留輕度 roast，但重點是把對方拉回正軌，而不是跟 `roast` 一樣兇。
...
本次 injected tone 的辨識策略採用「已知 prompt 字串比對」：

- 若第一個 part 的 `text` 等於任何 registry 內 preset 的 `prompt`，視為 plugin 注入的 tone
- 不額外加入 metadata marker

原因是目前 plugin 已採用字串比對處理重複注入，而 metadata marker 是否能在 OpenCode 整條訊息流中穩定保存，尚未在本 repo 中被驗證。先沿用字串比對能把風險留在可理解範圍內；極少數使用者若真的把完整 tone prompt 原封不動貼成第一個 part，會撞到這條規則，但這個風險在目前產品邊界內可接受。
...
1. 讀取完整 state
2. 找出第一個 user message 與其第一個 part；若不存在則直接 return
3. 依 `activeTone` 從 registry 取得 prompt
4. 檢查第一個 part 是否為任何已知 preset prompt
5. 若 `pluginEnabled && roastEnabled` 為 `false`：
   - 若第一個 part 是已知 preset prompt，將其移除
   - 否則直接 return
6. 若目前 enabled 為 `true` 且第一個 part 已是目前 active tone 的 prompt，直接 return
7. 若目前 enabled 為 `true` 且第一個 part 是其他已知 preset prompt，將其替換成目前 active tone 的 prompt
8. 若目前 enabled 為 `true` 且第一個 part 不是已知 preset prompt，插入對應 tone part

這代表 tone 切換或停用都要立即影響目前 thread 後續請求，而不是只對新 thread 生效。重點是「重複注入判斷」要從「只看目前 prompt 是否已存在」升級為「辨識任何已知 injected preset」。否則你從 `roast` 切到 `dry` 之後，server 會把新 tone 再疊在舊 tone 前面；而你把 tone 關掉時，舊人格還會陰魂不散地跟著送出去。
...
5. 若歷史第一個 user message 前面已有舊 injected preset，server 於下一次 transform 將其替換為 `deadpan`
6. 後續新請求只保留一份 `deadpan` prompt，不累積舊 preset

### Case C: 使用者關閉 tone 再重新打開

1. `roastEnabled=false` 時，若第一個 user message 前面已有已知 injected preset，server 於下一次 transform 將其移除
...
- 歷史中已有其他 preset prompt 時，會被替換而不是疊加
- `roastEnabled=false` 且歷史中已有 preset prompt 時，會移除該 prompt
